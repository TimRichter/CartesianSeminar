...
-- We defined foldTree mutually with foldTreeMap:
-- mutual
--   foldTree : {A B : Set} → (A → B) → (List B → B) → Tree A → B
--   foldTree f g (Leaf a)        = f a
--   foldTree f g (Node ts)       = g (foldTreeMap f g ts)
...
-- Again, we define type families for the conditions

FoldTreeUPCond1 : {A B : Set} → (Tree A → B) → (A → B) → Set
FoldTreeUPCond1 h f = h ∘ Leaf =∙= f

FoldTreeUPCond2 : {A B : Set} → (Tree A → B) → (List B → B) → Set
FoldTreeUPCond2 h g = h ∘ Node =∙= g ∘ mapL h

FoldTreeUPCond : {A B : Set} → (Tree A → B) → (A → B) → (List B → B) → Set
FoldTreeUPCond h f g = (h ∘ Leaf =∙= f)  ×  (h ∘ Node =∙= g ∘ mapL h) -- FoldTreeUPCond1 h f × FoldTreeUPCond2 h g

-- to prove foldTreeUP, we need that mapL preserves _=∙=_ !
...
-- We will also need that =∙= is an equivalence relation
...
-- Moreover, we need to prove that for any function g, precomposition with
...
-- The final preliminary before proving the universal property is the following
-- lemma, which follows easily from mapPres=∙= and the property that
-- foldTreeMap f g =∙= mapL (foldTree f g) we proved earlier.
...
foldTreeUP : {A B : Set} → (h : Tree A → B) → (f : A → B) → (g : List B → B) →
             h =∙= foldTree f g    ⇔   FoldTreeUPCond h f g
foldTreeUP h f g = Mk⇔ to from where
  to2 : h =∙= foldTree f g → FoldTreeUPCond2 h g
  to2 hEfTfg =
    h ∘ Node
    ...
    g ∘ mapL h
      QED=∙=
  to : h =∙= foldTree f g → FoldTreeUPCond h f g
  to hEfTfg = ((λ a → hEfTfg (Leaf a)) , to2 hEfTfg)

  -- as we defined foldTree mutually with foldTreeMap, we are bound to
  -- also define from (which proves a property of foldTree) mutually with fromMap:
  mutual
    from : FoldTreeUPCond h f g → h =∙= foldTree f g
    ...
    fromMap : FoldTreeUPCond h f g → mapL h =∙= foldTreeMap f g
    fromMap (c1 , c2) [] = refl
    fromMap (c1 , c2) (t ∷ ts) = 
       h t ∷ mapL h ts
