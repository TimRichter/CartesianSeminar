...
-- Vorschlag Sebastian 31.01.
{-
foldTreeMap' : {A B : Set} → (A → B) → (List B → B) →
               ((Tree A) → B) → List (Tree A) → List B
foldTreeMap' f g _ [] = []
foldTreeMap' f g h (t ∷ ts) = h t ∷ foldTreeMap' f g h ts


foldTree' : {A B : Set} → (A → B) → (List B → B) → Tree A → B
foldTree' f g (Leaf a)        = f a
foldTree' f g (Node ts)       = g (foldTreeMap' f g (foldTree' f g) ts)

-- still problematic for the termination checker
-}
...
mapLPres=∙= f g fEEg (a ∷ as) =
  mapL f (a ∷ as)    =⟨ refl ⟩
  f a ∷ mapL f as    =⟨ cong (_∷ mapL f as) (fEEg a) ⟩
  g a ∷ mapL f as    =⟨ cong (g a ∷_) (mapLPres=∙= f g fEEg as) ⟩
  g a ∷ mapL g as    =⟨ refl ⟩
  mapL g (a ∷ as)    QED
...
pre∘Pres=∙= : {A B C : Set} → (g : A → B) → {f h : B → C} →
               f =∙= h → f ∘ g =∙= h ∘ g
pre∘Pres=∙= g fEh a = fEh (g a)

post∘Pres=∙= : {A B C : Set} → (g : B → C) → {f h : A → B} →
               f =∙= h → g ∘ f =∙= g ∘ h
post∘Pres=∙= g fEh a = cong g (fEh a)
...
#+end_src agda2
** Session January 17th
*** did the derivation of ft (Marcus on whiteboard)
*** discussed associativity of function composition
*** again discussed _==_ vs _=∙=_
*** see [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230117_1.jpg][Whiteboard_CS_230117_1.jpg]], [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230117_2.jpg][Whiteboard_CS_230117_2.jpg]], [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230117_3.jpg][Whiteboard_CS_230117_3.jpg]]  
** Session January 24th
*** began reading §5
*** there is an open claim in the derivation of radixsort from treesort in §4, namely
  mapL (treesort ds) . ptn d = ptn d . treesort ds          (1)
*** we discussed this and Gibbons' remark that it expresses "stability" of ptn d
*** we generalised (treesort ds) to an arbitrary "sort" function: it should hold
  mapL sort . ptn d = ptn d . sort                          (1')
*** in essence, the property says that ptn d, when putting elements of its input list
into "bucket lists", it does not change the order of the elements: two elements landing
in the same "bucket" will appear there in the same order they appeared in the input list
*** Gibbons claims that equation (1) above follows from
  filter p . flatten = flatten . mapt (filter p)            (2)
and
  mapt (filter p) . mktree ds = mktree ds . filter p        (3)
*** we made a detailed type analysis of (1) resp. (1') and (2) and gave an example for
a typical (ptn d) where d is "map an integer to its next-to-last decimal digit"
(see [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230124_1.jpg][Whiteboard_CS_230124_1.jpg]] ) and made a type analysis of (3)
(see [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230124_2.jpg][Whiteboard_CS_230124_2.jpg]] ).
*** for (3) we need the map for trees (mapt), which wasn't defined yet. So we
gave a definition (see  [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230124_3.jpg][Whiteboard_CS_230124_3.jpg]] ). We also discussed that
this definition will not be accepted by the agda termination checker.
Exercise: define mapt in Agda (without using {-# TERMINATION #-} !)
** Session January 31th
*** discussed definition of mapt : finer pattern matching (also on the
list argument of Node) would not solve the problem - mutual definition
(along the lines of foldTree) would (see [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230131_1.jpg][Whiteboard_CS_230131_1.jpg]] )
*** but we can use the universal property of foldTree: the equations we would
have liked to give as a definition of mapt are
  mapt f ∘ Leaf =∙= Leaf ∘ f
  mapt f ∘ Node =∙= Node ∘ mapL (mapt f)
foldTreeUP shows that these conditions are equivalent to mapt f =∙= foldTree (Leaf ∘ f) Node
So it is ok to define mapt by this equation
*** We furthermore discussed the structure of the rest of the paper:
**** We have to prove  the "stability condition" for ptn we discussed earlier
  (1) ∀ d, ds .    mapL (treesort ds) ∘ ptn d = ptn d ∘ treesort ds
  This follows from the following equations
  (2) ∀ p .   filter p ∘ flatten = flatten ∘ mapt (filter p)
  (3) ∀ p, ds . mapt (filter p) ∘ mkTree ds = mkTree ds ∘ filter p
  So it remains to show (2)+(3) => (1) , (2) and (3).
**** We begin with (3): idea ist to use fusion, but the equation is not
in the right form for this. Will again use comp to get rid of the ds argument
and make fusion applicable. (see [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230131_2.jpg][Whiteboard_CS_230131_2.jpg]] )
** Session February 7th
*** discussed the equation
filter p ∘ flatten = flatten ∘ mapTree (filter p)
of page 6. See [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230207_1.jpg][Whiteboard_CS_230207_1.jpg]]  and  [[https://timrichter.github.io/CartesianSeminar/2210/Whiteboard_CS_230207_2.jpg][Whiteboard_CS_230207_2.jpg]]
** Code from the appendices (Tim 13.01.23 and 19.01.23)
*** Appendix A

#+begin_src agda2
-- this is what we called (1) in session of January 24th

mapTreesortPtnLemma : {A B : Set} → {{ finB : FinOrd B }} → (d : A → B) → (ds : List (A → B)) →
                      mapL (treesort ds) ∘ ptn d =∙= ptn d ∘ treesort ds
mapTreesortPtnLemma d ds as = {!!}

-- we define mapTree via foldTree

mapTree : {A B : Set} → (A → B) → Tree A → Tree B
mapTree f = foldTree (Leaf ∘ f) Node

-- it should follow from (2) that

filterFlattenLemma : {A : Set} → (p : A → Bool) → filter p ∘ flatten =∙= flatten ∘ mapTree (filter p)
filterFlattenLemma = {!!}
...
-- so Fin (Succ n) has exactly one more element than Fin n.
...
--     allA : List A
--     each∈ : ∀ a → a ∈ allA
--     uniqueness : length allA == card


-- mapL preserves composition (not actually needed here...)
...
-- there is a largest (or "last") element in any Fin (Succ m)

finLast : (m : ℕ) → Fin (Succ m)
finLast Zero     = FZero
finLast (Succ m) = FSucc (finLast m)

-- for elements of Fin (Succ n)  (which correspond to 0, 1, ... , n)
-- we define their "complements" (n, n-1 , ... 0)

finCompl : {m : ℕ} → Fin (Succ m) → Fin (Succ m)
finCompl {m} FZero = finLast m
finCompl {Succ m} (FSucc x) = FSucc (finCompl x)
...
divMod : (x : ℕ) → (m : ℕ) → {m' : ℕ} → {m == Succ m'} →
         divModFam x m
divMod Zero     (Succ m) = 0 ** (FZero ** refl)
divMod (Succ x) (Succ m) with divMod x (Succ m) {m} {refl}
... | d' ** (pr₃ ** pr₄) = {!!}


_div_ : ℕ → (n : ℕ) → {m : ℕ} → {n == Succ m} → ℕ
x div Succ m = pr₁ (divMod x (Succ m) {m} {refl}) where
  open Σ

_mod_ : ℕ → (n : ℕ) → {m : ℕ} → {n == Succ m} → Fin n
x mod Succ m = pr₁ (pr₂ (divMod x (Succ m) {m} {refl})) where
  open Σ


#+end_src agda2
** Consultation Feb 28th
*** foldr / foldList / fold in general : order of arguments  ?
**** matter of taste how to order the constructors
**** but there are other functions called folds, like e.g.
     foldl in Haskell.
#+begin_src agda2
...
*** discussed difference "logical" vs "type" reading
*** different equalities
see list January 3rd
equality is getting harder here!
look at the types!
